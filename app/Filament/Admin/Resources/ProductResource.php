    ...
    public static function form(Form $form): Form
  {
        ...
        Forms\Components\TextInput::make('price_dollars')
          ->label('Price (USD)')
              ...
              ->numeric()
                ->prefix('$')
           ->dehydrated(false)
            ->afterStateHydrated(fn ($state, $record, callable $set) => $set('price_dollars', $record ? $record->price / 100 : 0))
              ->live(onBlur: true)
              ->afterStateUpdated(fn ($state, callable $set) => $set('price', (int) round(((float) $state) * 100))),
            Forms\Components\Hidden::make('price'),
            Forms\Components\TextInput::make('emoji')->maxLength(8),
    ...
    public static function table(Table $table): Table
    {
       ...
       Tables\Columns\TextColumn::make('stock')
           ...
           Tables\Filters\SelectFilter::make('badge')->options(['new' => 'New', 'hot' => 'Hot']),
